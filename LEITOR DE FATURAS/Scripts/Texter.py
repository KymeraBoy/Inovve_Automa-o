...
def extrair_municipios_unicos(matriz_dados):
    municipios = set()  # Usa um conjunto (set) para evitar repetições

    # Percorre cada vetor/lista dentro da matriz
    for vetor in matriz_dados:
        # Percorre cada dicionário do vetor
        for item in vetor:
            if "MUNICÍPIO" in item:
                valor = item["MUNICÍPIO"]
                # Adiciona apenas se não for um valor nulo/indefinido
                if valor and valor != "SEM_MUNICIPIO":
                    municipios.add(valor)
...
#Apgar esee texto
    ...
    municipios_unicos = extrair_municipios_unicos(matriz)
    print(f"\nMunicípios únicos encontrados: {municipios_unicos}")
