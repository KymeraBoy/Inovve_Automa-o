...
def _extract_old_qip_payload(first_table: list[list[object]]) -> tuple[str, str, str, pd.DataFrame] | None:
	...
	return mes, ano, municipio, pd.DataFrame(data_rows, columns=headers)
	...
	try:
		with pdfplumber.open(pdf_path) as pdf:
			...
			if first_page_tables:
				old_payload = _extract_old_qip_payload(first_page_tables[0])
